...
class FBAuthManager: ObservableObject {
    ...
    func authenticateWithFirebase(accessToken _: AccessToken) {
    ...
    func getFBProfile(completion: @escaping (_ fbUser: [String: Any]?) -> Void) {
        GraphRequest(graphPath: "me", parameters: ["fields": "name, email"]).start { _, result, error in
            if error != nil {
